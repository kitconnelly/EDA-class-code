...
# data for today
metro_nash_schools <- read_csv("mod3/metro-nash-schools.csv")
...
- What is the data's structure? 
  - 168 obs of 35 variables
- What kind of information does it contain? 
  - Demographic information for multiple schools
- What is the unit of analysis?
  - Each obs is a different school
- What kind of questions might we be able to ask about this data? 

```{r}

dim(metro_nash_schools)
str(metro_nash_schools)
names(metro_nash_schools)
summary(metro_nash_schools)
...
- What does the variable ``economically disadvantaged` seem to represent?
  - How many students are at an economic disadvantage at each school.
- Are there any missing data for this variable? Is this variable an integer?
  - Yes, 1 row has a missing value. It is a double.

```{r}

sum(is.na(metro_nash_schools$`Economically Disadvantaged`))
typeof(metro_nash_schools$`Economically Disadvantaged`)

```

- What is the total population of students per school? Do we need to add this variable?
  - Adding the variable will help us calculate proportions for demographics.
- How is gender measured? 
  - Male or Female, count of each

```{r}

metro_nash_schools <- metro_nash_schools %>% mutate(total = Male + Female)
...
metro_nash_schools <- metro_nash_schools %>% mutate(econ_dis_prop = `Economically Disadvantaged` / total)
...
lowest <- metro_nash_schools %>% arrange(econ_dis_prop) %>% head()
highest <- metro_nash_schools %>% arrange(desc(econ_dis_prop)) %>% head()
...
```{r}
metro_nash_schools <- metro_nash_schools %>% mutate(lim_eng_prop = `Limited English Proficiency` / total)
```
...
ggplot(metro_nash_schools, aes(x=econ_dis_prop, y=lim_eng_prop, color = `School Level`)) +
  geom_point()
...
ggplot(metro_nash_schools, aes(x=econ_dis_prop, y=lim_eng_prop, color = `School Level`)) +
  geom_point() + 
  xlab('Economically Disadvantaged Students') +
  ylab('Students with Limited English Proficiency') +
  ggtitle('Limited English Proficiency and the Economic Status', subtitle = 'Nashville Metro Schools')
...
metro_nash_schools <- metro_nash_schools %>% mutate(prop_white = White / total)

ggplot(metro_nash_schools) +
  geom_point(aes(prop_white, econ_dis_prop, color = `School Level`)) +
  xlab('% White Students') +
  ylab('% Economically Disadvantaged Students') +
  ggtitle('Percentage of White Students and the Economic Status', subtitle = 'Nashville Metro Schools') +
